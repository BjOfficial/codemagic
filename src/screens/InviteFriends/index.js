...
const InviteFriends = () => {
      ...
      try {
        PermissionsAndroid.request(
          ...
          if (res == "granted") {
            ...
            Alert.alert(
              "To help you invite friends and family on Azzetta, allow Azzetta access to your contacts. Go to your devices Settings > Permissions, and turn Contacts on."
            );
  ...
  const loadContactList = async (mycontacts, resContacts) => {
    let responseContactList = resContacts.data.map(
      ({ phone_number }) => phone_number
    );
    let localContactList = mycontacts.map(({ phone_number }) => phone_number);

    let finalContactList = resContacts.data.filter(
      ({ phone_number }, index) => {
        if (localContactList.includes(phone_number)) {
          let localName = mycontacts.filter(
            (ele) => ele.phone_number == phone_number
          );
          resContacts.data[index].localName = localName
            .map(({ name }) => name)
            .toString();
        }

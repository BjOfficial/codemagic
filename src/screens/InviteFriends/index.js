...
import { invite_friends, whatsapp_icon, copy_icon, network_icon,networkadded } from '@constants/Images';
        ...
        <View style={styles.container}>
            ...
            <ScrollView>
                ...
                <SearchInput placeholder="search for name,number" value={searchvalue} onChangeText={(data) => setSearchvalue(data)} onPress={() => searchClick()} />
                    ...
                    {caontactList && caontactList.map((contact) => {
                        return (
                            <View style={styles.contactGroup}>
                                <View style={{ flex: 0.2 }}>
                                    <View style={[styles.contactIcon,{backgroundColor:contact.backgroundColor}]}><Text style={styles.contactIconText}>{contact.aliasName}</Text></View>
                                    {contact.remainder&&<Image source={network_icon} style={styles.network_icon} />}</View>
                                <View style={{ flex: 0.55 }}>
                                    <View style={{ flexDirection: 'column' }}><Text style={styles.contactName}>{contact.contactName}</Text>
                                        <Text style={styles.contactnumber}>{contact.contactNumber}</Text></View></View>
                                <View style={{ flex: 0.25,flexDirection:'row',alignItems:'center',justifyContent:'center' }}>
                                    {contact.invited?
                                    <TouchableOpacity><Image source={networkadded} style={styles.netword_added_icon}/></TouchableOpacity>:
                                    <ThemedButton title="Invite" mode="outlined" buttonStyle={{ padding: 0,margin:0 }} labelStyle={{ fontSize: font12 }} color={colorLightBlue}></ThemedButton>}
                                </View>
                            </View>
                        )
                    })}
